...
As we are using spring boot so we don`t need to run/maintain any server explicitly, spring boot has its own tomcat server inbuilt which runs on 8080 port by default.
If we want to run this application on different port we just need to define below in the application.properties file.
